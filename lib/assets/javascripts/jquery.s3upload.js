...
(function($){
  $.fn.s3upload = function( settings ) {
    var config = {
      ...
      path: "/flash/s3upload.swf",
      prefix: "",
      element: "<input type='text' />",
      signature_url: "/policy.xml",
      required: false,
      ...
      swfobject.embedSWF(config.path, fv.id, "100%", "100%", "9.0.0" , false, fv, params, false, function(e){
        if( e.success ) {
          ...
          swf.onenabled = function() {
            ...
            // TODO: this fails in safari [object NPMethod]
            if( !_.isFunction( swf.upload ) ) {
          ...
          swf.onselect = function(name,size,type) {
            ...
            if( $.isFunction( config.onselect ) )
              def = config.onselect.call(el,swf.info,swf);
            if( def ) {
