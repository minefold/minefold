...
paid_users = (
  Stripe::Customer.all(count: 100).data.
      map{|c| p c['description'];  c['description'] }.map{|d| id, username = d.split('-'); { id: id, username: username } } +
  Stripe::Charge.all(:count => 100).data.
      map{|c| p c['description']; c['description'] }.compact.select{|c| c.include? '-' }.map{|d| d.split('-').last }.map{|d| {id: d.split(':').first} }
)
